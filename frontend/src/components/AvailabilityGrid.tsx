...
interface Props {
  ...
  refreshKey?: number;
  userBookings?: { resource_id: string; slot_start: string; slot_end: string }[];
  userWaitlist?: string[];
}
...
export function AvailabilityGrid({ resource, onBook, refreshKey = 0, userBookings = [], userWaitlist = [] }: Props) {
  const { user } = useAuth();
  ...
  const isUserBooking = (slotStart: string) => {
    const normalized = normalizeDate(slotStart);
    return userBookings.some(b => normalizeDate(b.slot_start) === normalized && b.resource_id === resource.id);
  };

  const isUserWaitlisted = (slotStart: string) => {
    const normalized = normalizeDate(slotStart);
    return userWaitlist.some(w => normalizeDate(w) === normalized);
  };

  // Initialize joinedWaitlist from userWaitlist prop
  useEffect(() => {
    if (userWaitlist.length > 0) {
      const initialSet = new Set<string>();
      userWaitlist.forEach(w => {
        const matchingSlot = slots.find(s => normalizeDate(s.slot_start) === normalizeDate(w));
        if (matchingSlot) {
          initialSet.add(matchingSlot.slot_start);
        }
      });
      if (initialSet.size > 0) {
        setJoinedWaitlist(initialSet);
      }
    }
  }, [userWaitlist, slots]);
      ...
      {!loading && !error && slots.length > 0 && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10 }}>
          {slots.map(slot => {
            ...
            const isOngoing     = slotStart <= now && slotEnd >= now;
            ...
            if (isBooked) {
              const isOwnBooking = canBook && isUserBooking(slot.slot_start);
              return (
            ...
            const slotStyle: CSSProperties = isPast ? {
              ...
              borderRadius: 14, padding: '12px 10px', textAlign: 'center',
              background: '#e8f5e9', border: '1.5px solid #a5d6a7', cursor: 'pointer',
            ...
            return (
              ...
              >
                <p style={{ fontSize: 13, fontWeight: 600, margin: 0, color: isPast ? '#b0bac6' : isOngoing && available ? '#2e7d32' : available ? '#1e7a88' : '#c0402c' }}>
                  {formatTime(slot.slot_start)}
                </p>
                <p style={{ fontSize: 11, margin: '3px 0 0', color: isPast ? '#c8d0d8' : isOngoing && available ? '#4caf50' : available ? '#4ca8b0' : '#e47a67' }}>
                  {isPast ? 'Past' : isOngoing && available ? 'Ongoing' : isOngoing ? 'In use' : available ? 'Available' : 'Booked'}
                </p>
              </button>
            );
          })}
