...
async def get_resource(
    ...
    try:
        ...
        logger.error(f"Database error in get_resource({resource_id}): {e}")
...
async def search_resources(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to search resources.")
...
async def get_availability(
    ...
    try:
        ...
        logger.error(f"Database error in get_availability({resource_id}, {date}): {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to get availability.")
...
async def check_in(
    ...
    try:
        ...
        logger.error(f"Error in check_in({qr_token}): {e}", exc_info=True)
...
async def websocket_endpoint(websocket: WebSocket, resource_id: str):
    ...
    try:
        ...
        logger.error(f"WebSocket error for resource {resource_id}: {e}")
...
async def join_waitlist(
    ...
    try:
        ...
        await db.rollback()
        logger.error(f"Database error in join_waitlist: {e}", exc_info=True)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to join waitlist.")
    except Exception as e:
        await db.rollback()
        logger.error(f"Error in join_waitlist: {e}", exc_info=True)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to join waitlist.")
...
async def get_my_waitlist(
    ...
    try:
        ...
        logger.error(f"Database error in get_my_waitlist: {e}", exc_info=True)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to retrieve waitlist.")
...
async def get_room_overview(
    ...
    try:
        ...
        logger.error(f"Database error in get_room_overview: {e}", exc_info=True)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to retrieve room overview.")
    except Exception as e:
        logger.error(f"Error in get_room_overview: {e}", exc_info=True)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to retrieve room overview.")
...
async def get_utilization(
    ...
    try:
        ...
        logger.error(f"Database error in get_utilization: {e}", exc_info=True)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to retrieve utilization stats.")
...
async def get_heatmap(
    ...
    try:
        ...
        logger.error(f"Database error in get_heatmap: {e}", exc_info=True)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to retrieve heatmap data.")