"""
SportsAdapter — Concrete adapter for Sports Complex resources.

In prototype: queries our own PostgreSQL database.
In production: would call the sports complex booking API.

Pattern: Adapter (GoF) — same interface as StudyRoomAdapter, different
         class name for future extensibility (e.g., weather-dependent
         outdoor court availability, equipment rental checks).
"""
...
class SportsAdapter(ResourceAdapter):
    """
    Adapter for Sports Complex Booking System.
    In prototype: queries our own DB.
    In production: would call sports_complex_api.check_court(slot).
    """
